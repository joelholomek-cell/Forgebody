...
const GLASS = `body{background:#0a0a0a;margin:0} .fb-bg::before{content:'';position:fixed;inset:0;background:radial-gradient(ellipse 80% 55% at 10% 5%,rgba(204,255,0,0.13) 0%,transparent 55%),radial-gradient(ellipse 60% 45% at 90% 85%,rgba(120,220,0,0.08) 0%,transparent 50%);pointer-events:none;z-index:0} input,select,textarea{color:#fff!important;-webkit-text-fill-color:#fff!important} input::placeholder,textarea::placeholder{color:rgba(255,255,255,0.3)!important} select option{background:#111;color:#fff} *{box-sizing:border-box;-webkit-tap-highlight-color:transparent} @keyframes bounce{0%,80%,100%{transform:scale(0)}40%{transform:scale(1)}} @keyframes gradSpin{0%{background-position:0% 50%}50%{background-position:100% 50%}100%{background-position:0% 50%}} @keyframes fadeUp{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}} @keyframes slideIn{from{transform:translateX(-100%)}to{transform:translateX(0)}} @keyframes scaleIn{from{opacity:0;transform:scale(0.96)}to{opacity:1;transform:scale(1)}} @keyframes slideUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}} @keyframes glowPulse{0%,100%{box-shadow:0 0 12px rgba(204,255,0,0.15)}50%{box-shadow:0 0 28px rgba(204,255,0,0.4)}} @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-5px)}} @keyframes heartbeat{0%,100%{transform:scale(1)}14%{transform:scale(1.07)}28%{transform:scale(1)}} @keyframes shimmer{0%{opacity:0.5}50%{opacity:1}100%{opacity:0.5}} @keyframes spin360{from{transform:rotate(0deg)}to{transform:rotate(360deg)}} .anim-fadeUp{animation:fadeUp 0.4s ease both} .anim-scaleIn{animation:scaleIn 0.25s ease both} .anim-slideUp{animation:slideUp 0.35s ease both} .anim-float{animation:float 3s ease-in-out infinite} .anim-glowPulse{animation:glowPulse 2s ease-in-out infinite} .anim-heartbeat{animation:heartbeat 1.5s ease-in-out infinite} .press{transition:transform 0.12s ease} .press:active{transform:scale(0.97)} .card-anim{animation:scaleIn 0.3s ease both}`;
...
“Barbell Bench Press”:{img0:“https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/Barbell_Bench_Press_-*Medium_Grip/0.jpg”,img1:“https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/Barbell_Bench_Press*-*Medium_Grip/1.jpg”},
“Incline Barbell Press”:{img0:“https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/Barbell_Incline_Bench_Press*-*Medium_Grip/0.jpg”,img1:“https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/Barbell_Incline_Bench_Press*-*Medium_Grip/1.jpg”},
...
“Dips (chest lean)”:{img0:“https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/Dips*-*Chest_Version/0.jpg”,img1:“https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/Dips*-*Chest_Version/1.jpg”},
...
“Dips (tricep focus)”:{img0:“https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/Dips*-*Triceps_Version/0.jpg”,img1:“https://raw.githubusercontent.com/yuhonas/free-exercise-db/main/exercises/Dips*-_Triceps_Version/1.jpg”},
...
<style>{`@keyframes heroGlow{0%,100%{opacity:0.6}50%{opacity:1}} @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}} @keyframes shimmer{0%{background-position:-200% 0}100%{background-position:200% 0}}`}</style>
...
contextLine = ` Previous phase ended with: ${summary} Now INCREASE intensity, vary exercises, progress from this point.`;
}
...
<style>{`@keyframes robotBounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-10px)}} @keyframes earWiggle{0%,100%{transform:rotate(0deg)}25%{transform:rotate(-20deg)}75%{transform:rotate(20deg)}} @keyframes stepSpin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}} @keyframes stepFade{from{opacity:0;transform:translateX(-8px)}to{opacity:1;transform:translateX(0)}}`}</style>
...
{STEPS.map((*,i)=><div key={i} style={{flex:1,height:“3px”,borderRadius:“2px”,background:i<=step?”#CCFF00”:“rgba(255,255,255,0.1)”,transition:“background 0.3s”}}/>)}
</div>
